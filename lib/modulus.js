...
var defaults = {
    ...
    _validateReadModules: function(modules){
        ...
        for(var i=0; i < modules.length; ++i){
            var module = modules[i];
            var modulesWithSameName = findModulesByProp("name", module.name);
            var modulesWithSameFunctionName = findModulesByProp("functionName", module.functionName);
    ...
    _handleFailedValidation: function(errors, errorback){
        if(errors.length > 0){
            throw errors;
        }
    },
...
modulus.build = function(configOverrides, callback, errorback){
    ...
    config._getModuleFilePaths(config.baseDirectory, config.modulePattern)
        ...
        .then(function(modules){
            ...
            config._handleFailedValidation(errors, errorback);
            callback();
        })
        .fail(function(err){
            //log('ERROR: %s', err);
