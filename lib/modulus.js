...
var defaults = {
    ...
    _processDistConfigs: function(config, allModules){
        ...
        for(var distFileName in dist.files){
            ...
            this._processDistConfig(distFileName, distFileConfig, config, allModules);
        }
    },
    ...
    _processDistConfig: function(distFileName, distFileConfig, config, allModules){
        ...
        log('found %s dependencies for distFileName %s', allDependencies.length, distFileName);
        //create a big string
        var distFileContents = this._combineModuleContentsToOneString(allDependencies);
    ...
    _recursivelyFindDependencies: function(dependencyModules, allModules){
        ...
        //todo: worry about infinite loops with modules that depend on each other.
        for(var i=0; i < dependencyModules.length; ++i){
            ...
            //foundModules.push(dependencyModule);
...
modulus.build = function(configOverrides, callback, errorback){
    ...
    config._getModuleFilePaths(config.baseDirectory, config.modulePattern)
        ...
        .then(function(modules){
            ...
            config._processDistConfigs(config, modules);
