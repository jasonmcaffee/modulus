...
//            }
    ...
    /**
     * TODO: OMG HANDLE REQUIRES DONT ASSUME THEY ARE ALL DEFINES GEEZ!!!
     * TODO: handle multiple modules defined in a file.
    ...
    _rewriteAModulesContentsToUseExplicitDefines: function(module){
        ...
        var modulusFunctionCallRegex = /((?:m|modulus)\((?:\s*?)function(?:\s*\w*\s*)\()/g;    //gets the name of the module function.
        var moduleNameRegex = /(?:m|modulus)\((?:\s*?)function(?:\s*(\w*)\s*)\(/g;
        var depsRegex = /(?:m|modulus)\((?:\s*?)function(?:\s*\w*\s*)\((.*)\)/g;
        //var modulusCallIndex = contents.search(modulusFunctionCallRegex);
        var matches = modulusFunctionCallRegex.exec(contents);//funcString.match(reg)[0];
        ...
        if(matches){
            for(var i=0; i<matches.length;++i){
                ...
                var moduleName = moduleNameResult ? moduleNameResult[0] : '';
                log('module name: ' + moduleName + ' found in m function call string: ' + modulusFunctionCallString);
                if(moduleName){ //define
                    if(i > 0){throw 'Error: Only 1 define is allowed per file. More than 1 module define found in module file: ' + module.filePath;}
    ...
    /**
     * TODO: Sort by number of deps? NO. sort by dependency . e.g. dep should go above module.
     * reorders modules.
...
//        }
        ...
        //todo: requires need to be at the bottom.
        return reordered;
    },
    /**
