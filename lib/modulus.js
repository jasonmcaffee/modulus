...
var defaults = {
    ...
    //the files we want to build and distribute.
//    dist:{
//        directory: 'dist',
//        files:{
...
//        }
//    },
//    map:{  TODO: implement the mapping config
//        requestsFor:{
//            'moduleA':{
//                from:{
//                    'moduleB':{
...
//    },
    ...
    /**
     * When true, rewrites calls to modulus so that they include moduleName and deps as a string and array.
     * e.g. m(function moduleA(moduleB){..}) becomes m('moduleA', ['moduleB'], function(modB){...}
     */
    protectAgainstMinification: true,

    /**
     * If you use a namespace object for module definitions, you must specify the name of the namespace
     * e.g. if we use ns.moduleA = function(dep1, dep2), then we must specifiy 'ns' as the namespace name.
     */
    moduleContextName: undefined,
    ...
    _createModuleMetadata: function(path, fileContents){
        ...
        if(shimConfigEntry){
            ...
            //by validating funcName, we help ensure runtime will run smoothly. funcName is not used in the build otherwise.
...
//    },
    ...
    /**
     * DEPRECATED: NOT NEEDED AT BUILD TIME. PEOPLE CAN HAVE ANY SIGNATURE THEY WANT.
     * by validating funcName, we help ensure runtime will run smoothly. funcName is not used in the build otherwise.
...
//    },
    ...
    _determineModuleDependencies: function(name, contents, path){
        //var reg = /\((.*)\)/g;   //didn't allow for m(function moduleA(moduleB){...});
        //better one in progress (?:m|modulus)\((?:\s*?)function(?:\s*(.*)\s*)\(
        var reg =/function(?:.*)\((.*)\)/g;
        var funcString = contents;
        var matches = reg.exec(funcString);//funcString.match(reg)[0];
        var paramsAsString = matches? matches[1] : '';
        paramsAsString = paramsAsString.replace(/\s/g, '').replace(/\n/g, '').replace(/\t/g,'');
        var deps;
        if(paramsAsString == ''){
            deps = [];
        }else{
    ...
    /**
     * makes sure that no duplicate modules exist, etc
...
//            }
    ...
    _processDistConfig: function(distFileName, distFileConfig, config, allModules){
        ...
        this._rewriteContentsToCreateModulusFunctionCallForShims(allUniqueOrderedDependencies);
        //loop through each module and potentially modify the contents (e.g. to protect against minification, etc)
        if(this.protectAgainstMinification){
            this._rewriteContentsToUseExplicitDefines(allUniqueOrderedDependencies);
        }
        log('protectAgainstMinification set to : ' + this.protectAgainstMinification);

        log('modules are: %s', stringifyModules(allUniqueOrderedDependencies));
    ...
    /**
     * Helps to avoid having to write a runtime configuration for shimmed modules.
     * @private
     */
    _rewriteContentsToCreateModulusFunctionCallForShims: function(modules){
        for(var i=0; i < modules.length; ++i){
            var module = modules[i];
            if(module.isShim){
                var newContents = '\n;m("'+module.name+'", '+JSON.stringify(module.dependencies)+ ', function(){return ' + module.name + ';});';
                module.contents = module.contents + newContents;
            }
        }
    ...
    /**
     * Used for protecting against minification.
     * Iterates over each module and rewrites the define so that it uses strings for moduleName and deps.
     * @param modules
     * @private
     */
    _rewriteContentsToUseExplicitDefines: function(modules){
        for(var i=0; i<modules.length; ++i){
            var module = modules[i];
            this._rewriteAModulesContentsToUseExplicitDefines(module);
        }
    },

    /**
     *
     * Rewrites the module contents (dist version) to use explicit defines.
     * ONLY 1 Define is allowed per file!!!
     * e.g.
     * m(function moduleA(dep1, dep2){...})
     * becomes
     * m('moduleA', ['dep1', 'dep2'], function moduleA(dep1, dep2){...});
     * Handles nested requires as well.
     * e.g.
     * modulus(function (dep1, dep2){...})
     * becomes
     * modulus(['dep1', 'dep2'], function(dep1, dep2){...});
     * @param module
     */
    _rewriteAModulesContentsToUseExplicitDefines: function(module){
        if(module.isShim){return;}//dont handle shims.
        log('----------------------------------');
        log('rewriting module : ' + module.name);
        var contents = module.contents;
        var originalContents = contents;
        //assume that the first function found in the file is the module
        var modulusFunctionCallRegex = /((?:m|modulus)\((?:\s*?)function(?:\s*\w*\s*)\(.*\))/g; // /((?:m|modulus)\((?:\s*?)function(?:\s*\w*\s*)\()/g;    //gets the name of the module function.

        var matches, noMatches = true, defineCalls =0;
        var explicitParams, newModulusFunctionCallString;
        while(matches = modulusFunctionCallRegex.exec(originalContents)){
            noMatches = false;
            //var modulusFunctionCallString = matches[i];
            var modulusFunctionCallString = matches[1];
            var moduleNameRegex = /(?:m|modulus)\((?:\s*?)function(?:\s*(\w*)\s*)\(/g;
            var moduleNameResult = moduleNameRegex.exec(modulusFunctionCallString);
            log('moduleNameResult length: ' +  (moduleNameResult ? moduleNameResult.length : null));
            var moduleName = moduleNameResult ? moduleNameResult[1] : '';
            log('module name: ' + moduleName + ' found in m function call string: ' + modulusFunctionCallString);
            if(moduleName){ //define
                if(defineCalls++ > 0){throw 'Error: Only 1 define is allowed per file. More than 1 module define found in module file: ' + module.filePath;}
                explicitParams = '("' + module.name + '",'+JSON.stringify(module.dependencies)+', ';
                //assume the first '(' is the modulus function call.
                newModulusFunctionCallString = modulusFunctionCallString.replace('(', explicitParams);
                contents = contents.replace(modulusFunctionCallString, newModulusFunctionCallString);
            }else{ //require      - rewrite sub requires so that they function after minification
                var depsRegex = /(?:m|modulus)\((?:\s*?)function(?:\s*\w*\s*)\((.*)\)/g;
                var depsMatch = depsRegex.exec(modulusFunctionCallString);
                var paramsAsString = depsMatch? depsMatch[1] : '';
                paramsAsString = paramsAsString.replace(/\s/g, '').replace(/\n/g, '').replace(/\t/g,'');
                if(paramsAsString){
                    var deps = paramsAsString.split(',');
                    explicitParams = '('+JSON.stringify(deps)+', ';
                    newModulusFunctionCallString = modulusFunctionCallString.replace('(', explicitParams);
                    contents = contents.replace(modulusFunctionCallString, newModulusFunctionCallString);
                }
            }
        ...
        //if no modulus functions were called, process as global functions.
        if(noMatches){//global function
            log('no modulus function calls found. assuming global function');
            var contextName = this.moduleContextName || '';
            var contextAndModuleName = contextName ? contextName + '.' + module.name : module.name;
            //handle global module functions.
            //myModule.module = {name:'myModule', deps:['dependency1', 'dependency2']}
            var metadataIndex = contents.indexOf(module.name+'.module'); //if there already is metadata, append to it. if not, create one.
            if(metadataIndex >=0){
                var newContent = '\n;'+contextAndModuleName+'.module.name = "'+module.name+'";'+
                    '\n'+contextAndModuleName+'.module.deps = '+JSON.stringify(module.dependencies)+';';
                contents = contents + newContent;
            }else{
                var newContent = '\n;'+contextAndModuleName+'.module = {name:"'+module.name+'", deps:' +JSON.stringify(module.dependencies)+'};';
                contents = contents + newContent;
            }
        }
    ...
    _excludeModules: function(distFileConfig, allModules, currentModules){
        ...
        var excludedModulesAndDependencies = exludedModules.concat(this._recursivelyFindDependencies(exludedModules, allModules));
        log('excluding %s modules', excludedModulesAndDependencies.length);
        for(var i =0; i < excludedModulesAndDependencies.length; ++i){
            var excludedModule = excludedModulesAndDependencies[i];
            log('excluding module name: '+ excludedModule.name);
        }
    ...
    _reorderModules: function(modules){
        ...
        function sort(a, b){
            ...
            //first check for shims since they are easier to order.
            //all shims go at the top.
            if(a.isShim && !b.isShim){
                return -1;
            }
            if(b.isShim && !a.isShim){
                return 1;
            }
            if(a.isShim && b.isShim){
                return 0;
            ...
            var allAdeps = self._recursivelyFindModuleNamesForAllDependencies(a, modules);
            var allBdeps = self._recursivelyFindModuleNamesForAllDependencies(b, modules);
            if(allAdeps.indexOf(b.name) >=0){
                //log(b.name + ' should be before ' + a.name);
                //a depends on b so b should be lower index than a
                return 1; //sort b to be lower index than a
            }

            if(allBdeps.indexOf(a.name) >=0){
               // log(a.name + ' should be before ' + b.name);
                //b depends on a so a should be lower index than b
                return -1; //sort a to be lower index than a
            }

            return 0;// a and b should be considered equal
        }

        reordered = modules.sort(sort);
        return reordered;
    },
    /**
     * Primarily needed for sorting function in _reorderModules
     * @param module
     * @param modules
     * @returns {Array} - module names. e.g. ['modA', 'modB']
     * @private
     */
    _recursivelyFindModuleNamesForAllDependencies : function(module, modules){
        //if(module.allNestedDepsStrArr){return module.allNestedDepsStrArr;}//memoization only. should not be used outside of this function.
        var allDeps = this._recursivelyFindDependencies([module], modules);
        var result = [];
        for(var i=0; i<allDeps.length; ++i){
            var item = allDeps[i];
            result.push(item.name);
        }
        //module.allNestedDepsStrArr = result;
        return result;
    },
